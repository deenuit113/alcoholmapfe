...
export const mypageEditSchema = (currentEmail: string, currentPassword: string, currentNickname: string, currentCapaSoju: number) => yup.object({
    email: yup
        ...
        .required('이메일을 입력하세요')
        .matches(emailRegex, '올바른 이메일 형식이 아닙니다.')
        .test('unique-email', '현재 이메일과 동일합니다. 다른 이메일로 시도하세요.', (value) => {
    ...
    password: yup
        ...
        .required('비밀번호를 입력하세요')
        .matches(passwordRegex, '비밀번호는 최소 8자 이상, 최소 하나의 알파벳 및 하나의 숫자, 특수문자를 포함해야 합니다.')
        .test('unique-password', '현재 비밀번호과 동일합니다. 다른 비밀번호로 시도하세요.', (value) => {
          return value === currentPassword;
        }),
    nickname: yup
        .string()
        .required('닉네임을 입력하세요.')
        .test('unique-nickname', '현재 닉네임과 동일합니다. 다른 닉네임으로 시도하세요.', (value) => {
          return value === currentPassword;
        }),
    ...
    capaSoju: yup
        .string()
        .required('주량을 입력하세요.')
        .test('unique-capaSoju', '현재 주량과 동일합니다. 다른 값을 넣어주세요.', (value) => {
          return value === currentPassword;
        }),
});